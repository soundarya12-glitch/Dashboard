...
export default function EditProfilePage() {
  ...
  useEffect(() => {
    ...
    const storedAvatar = localStorage.getItem(`avatar-${profileId}`);
    const storedAssigned = localStorage.getItem(`assigned-${profileId}`);
    const storedTags = localStorage.getItem(`tags-${profileId}`);
    ...
    <div className="p-6 w-80 bg-gray-900 text-white rounded mx-auto mt-8">
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {tags.map((t, i) => (
            <span key={i} className="bg-blue-600 px-2 rounded text-xs">
              {t}
            </span>
          ))}
        </div>
